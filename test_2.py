...
if __name__ == '__main__':
    ...
    all_targets = []
    # 遍历DataLoader
    for i, (image, image2,segment_image) in enumerate(tqdm.tqdm(data_loader)):
        # 将输入和目标数据移动到相同设备(CPU或GPU)
        image, image2,segment_image = image.to(device), image2.to(device),segment_image.to(device)
        # 前向传播获取预测结果
        outputs = net(image)
        # 后处理可视化
        _image = image2[0].cpu().numpy()
